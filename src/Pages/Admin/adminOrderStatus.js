...
const AdminOrderStatus = () => {
  ...
  const handleEditStatus = async () => {
    try {
      ...
      await axios.patch(
        `http://localhost:5000/orders/${selectedOrderId}/change-status`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      // After status update, fetch orders again to update the order list
      await fetchOrders();
      toast.success("Status Updated Successfully");
      setEditStatusVisible(false); // Close the edit status modal
    } catch (error) {
